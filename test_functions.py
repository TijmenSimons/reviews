...
def check_item(item1, item2):
    ...
    assert item1["max_claims"] == item2["max_claims"], "Incorrect max_claims"

    return True


def check_sale(sale1, sale2):
    """Check if 2 sales are equal"""

    assert sale1["item_id"] == sale2["item_id"], "Incorrect name"
    assert sale1["buyer"] == sale2["buyer"], "Incorrect is_hidden"
    assert sale1["description"] == sale2["description"], "Incorrect description"
